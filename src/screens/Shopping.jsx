    ...
    <div className="flex-1 flex flex-col bg-ink-100 relative">
      ...
      <div className="absolute inset-0 bg-ink-900/70 flex items-center justify-center px-6 animate-fade-in">
        <div className="w-full bg-white rounded-modal shadow-modal p-6 text-center animate-pop">
          ...
          <h2 className="text-[18px] font-extrabold text-ink-900 leading-snug mt-1">
            السماح لتطبيق جديد بالوصول إلى موقعك؟
          </h2>
          <p className="text-[12px] font-medium text-ink-500 leading-relaxed mt-2">
            نحتاج إذن تحديد الموقع الجغرافي لعرض المتاجر والمنتجات الأقرب إليك
            في مدينة {CITY} وحساب رسوم التوصيل بدقة.
          </p>
          <div className="space-y-2 mt-5">
            <button
              onClick={() => navigate("locationSuccess", {}, { replace: true })}
              className="w-full btn-primary btn-md"
            >
              السماح أثناء استخدام التطبيق
            </button>
            <button
              onClick={() => navigate("locationDenied", {}, { replace: true })}
              className="w-full btn-ghost btn-md"
            >
              عدم السماح الآن
            </button>
          </div>
    ...
    >
      ...
      {pin && (
        <div className="absolute left-1/2 top-[44%] -translate-x-1/2 -translate-y-full flex flex-col items-center animate-pop">
          {label && (
            <div className="bg-ink-900 text-white text-[11px] font-bold rounded-full px-3 h-7 flex items-center mb-1 shadow-elevated">
              {label}
            </div>
          )}
...
export function LocationSuccess() {
  ...
  return (
    <StateScreen
      ...
      primary={{
        label: "تأكيد هذا الموقع ومتابعة التسوق",
        onClick: () => navigate("addresses", {}, { resetTo: true }),
      }}
    >
      <div className="card p-4 text-right">
        ...
        <p className="text-[13px] font-bold text-ink-900 mt-1">
          {CITY}، حي المسبح، بالقرب من مدرسة ناصر
        </p>
        <p
          className="font-mono text-[11px] text-ink-400 bg-ink-100 rounded-lg px-3 py-1.5 mt-2 inline-block"
          dir="ltr"
        >
          N 13.5789° , E 44.0124°
        </p>
...
export function Addresses() {
  ...
  const [form, setForm] = useState({
    title: currentAddress?.title || "موقعي",
    details: currentAddress?.details?.replace(`، ${CITY}`, "") || "",
  ...
  const save = () => {
    ...
    if (form.details.trim().length < 5)
      er.details = "اكتب تفاصيل الموقع (الحي، الشارع، أقرب معلم)";
    if (!/^(\+?967)?7\d{8}$/.test(form.phone.replace(/\s/g, "")))
      er.phone = "رقم جوال صحيح يبدأ بـ 7 (9 أرقام)";
    setErrors(er);
    if (Object.keys(er).length)
      return showToast("أكمل بيانات الموقع", "danger");
    dispatch({
      type: "UPDATE_ADDRESS",
      patch: {
        title: form.title.trim(),
        details: `${form.details.trim()}، ${CITY}`,
  ...
  return (
    <div className="flex-1 flex flex-col bg-ink-50">
      <StatusBar />
      <TopBar
        title={canGoBack ? "تغيير موقع التوصيل" : "تحديد موقع التوصيل"}
        subtitle={`لكل حساب موقع توصيل واحد في ${CITY} يمكنك تحديثه في أي وقت`}
      />
      <div className="flex-1 overflow-y-auto scroll-thin px-5 py-4 space-y-4">
        <button
          onClick={() => navigate("mapPin")}
          className="w-full card p-4 flex items-center gap-3 text-right hover:border-primary transition"
        >
          ...
          <div className="flex-1">
            <div className="text-[14px] font-bold text-primary">
              تحديث الموقع من الخريطة (GPS)
            </div>
            <div className="text-[11px] font-medium text-ink-500">
              تحديد تلقائي لمنطقتك على الخريطة
            </div>
          </div>
          ...
          {currentAddress && !editing ? (
            <div className="card p-4 border-2 border-primary bg-primary-50/40 flex items-start gap-3 text-right">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div
                  className="text-[11px] font-bold text-primary mt-1 tabular"
                  dir="ltr"
                >
              ...
              <button
                ...
                className="text-[11px] font-bold text-secondary shrink-0"
              >
                تعديل يدوياً
              </button>
            </div>
            ...
            <div className="card p-4 space-y-3 animate-slide-up">
              ...
              <div>
                <label className="label">
                  التفاصيل (الحي، الشارع، أقرب معلم)
                </label>
                <input
                  className={`field ${errors.details ? "field-error" : ""}`}
                  placeholder="الحوبان، خلف مستشفى الثورة"
                  value={form.details}
              ...
              <div>
                ...
                <input
                  dir="ltr"
                  inputMode="tel"
                  className={`field text-left tabular ${errors.phone ? "field-error" : ""}`}
              ...
              <div className="flex gap-2">
                <button onClick={save} className="flex-1 btn-primary btn-md">
                  حفظ الموقع
                </button>
                {currentAddress && (
                  <button
        ...
        <p className="text-[10px] text-ink-400 leading-relaxed px-1">
          يُستخدم هذا الموقع لعرض المتاجر الأقرب وحساب التوصيل، ويُطلب تأكيد
          بيانات الاستلام عند كل طلب.
        </p>
      </div>
      <div className="px-5 pb-4 pt-2 bg-ink-50">
        <button
          onClick={confirm}
          disabled={!currentAddress || editing}
          className="w-full btn-primary btn-lg"
        >
          {canGoBack ? "حفظ واعتماد هذا الموقع" : "تأكيد الموقع ومتابعة التسوق"}
        </button>
      </div>
      <HomeIndicator />
    </div>
    ...
    <div className="flex-1 flex flex-col relative bg-ink-100">
      ...
      <div className="relative">
        ...
        <div className="px-4 pt-1 flex items-center justify-between">
          <button
            onClick={back}
            className="icon-btn bg-white shadow-card"
            aria-label="رجوع"
          >
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="text-[10px] font-bold text-ink-400">
                الموقع الحالي في {CITY}
              </p>
              <p className="text-[14px] font-bold text-ink-900">
                شارع جمال، جوار بريد تعز المركزي
              </p>
            ...
            <button
              onClick={() => {
                dispatch({
                  ...
                  patch: {
                    title: "موقعي",
                    details: `شارع جمال، جوار بريد تعز المركزي، ${CITY}`,
                  },
                });
...
const PROMO_TONES = {
  secondary: {
    card: "bg-gradient-to-l from-secondary to-secondary-400 shadow-accent",
    btn: "text-secondary",
  },
  primary: {
    card: "bg-gradient-to-l from-primary to-primary-500 shadow-brand",
    btn: "text-primary",
  },
  dark: {
    card: "bg-gradient-to-l from-ink-900 to-ink-700",
    btn: "text-ink-900",
  },
};
const PROMO_TRANSITION_MS = 2200;
const PROMO_AUTOPLAY_DELAY_MS = 3500;
function PromoCarousel({ promos, onAction }) {
  ...
  // التمرير البرمجي إلى إعلان معيّن مع دعم اختلاف قيمة scrollLeft في RTL بين المتصفحات
  const goTo = (i) => {
    ...
    if (track) {
      ...
      const step = maxScroll / Math.max(1, count - 1);
      const left = step * next;
      const targetLeft =
        getComputedStyle(track).direction === "rtl" ? -left : left;
      const startLeft = track.scrollLeft;
      const startedAt = performance.now();
      const duration = PROMO_TRANSITION_MS;
      ...
      const animate = (now) => {
        ...
        track.scrollLeft = startLeft + (targetLeft - startLeft) * eased;
        if (progress < 1) {
  ...
  const onScroll = () => {
    ...
    const nearest = Math.max(
      0,
      Math.min(count - 1, Math.round(position / step)),
    );
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => goTo(index + 1), PROMO_AUTOPLAY_DELAY_MS);
  ...
  return (
    ...
    >
      <div
        ref={trackRef}
        onScroll={onScroll}
        className="flex overflow-x-auto no-scrollbar snap-x snap-mandatory -mx-4 px-4 gap-3"
        ...
        {promos.map((p, i) => {
          ...
          return (
            <div
              ...
              aria-label={`إعلان ${i + 1} من ${count}: ${p.title}`}
            >
              <div
                className={`relative overflow-hidden rounded-modal text-white p-4 min-h-[132px] ${tone.card}`}
              >
                <Sparkles
                  className="absolute left-4 top-3 text-white/25"
                  size={80}
                  strokeWidth={1.2}
                />
                <span className="inline-block bg-white/20 rounded-full px-2.5 h-5 text-[10px] font-bold leading-5">
                  {p.badge}
                </span>
                <h2 className="text-[20px] font-black mt-2 leading-tight">
                  {p.title}
                </h2>
                <p className="text-[12px] font-medium text-white/90 line-clamp-2">
                  {p.text}
                </p>
                <button
                  onClick={() => onAction(p)}
                  className={`mt-3 h-8 px-4 rounded-full bg-white text-[12px] font-bold ${tone.btn}`}
                >
                  {p.cta}
                </button>
              </div>
        ...
        >
          {promos.map((p, i) => (
            <button
              ...
              onClick={() => goTo(i)}
              className={`h-1.5 rounded-full transition-all duration-300 ${i === index ? "w-5 bg-secondary" : "w-1.5 bg-ink-300"}`}
            />
          ))}
        </div>
      )}
...
export function Home() {
  ...
  const featured = useMemo(
    () =>
      PRODUCTS.filter(
        (p) =>
          !p.deleted && (cat === "all" || p.category === cat) && p.stock > 0,
      ),
    [cat, state.catalogVersion],
  );
  return (
        ...
        <div className="px-4 pb-4 relative">
          <div className="flex items-center justify-between">
            ...
            >
              ...
              <div>
                <p className="text-[10px] font-medium text-white/75 leading-none">
                  التوصيل إلى
                </p>
                <p className="text-[13px] font-bold flex items-center gap-1">
                  {currentAddress?.title?.replace(" (الرئيسي)", "")} ·{" "}
                  {currentAddress?.details?.split("،")[0]}
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                {cartCount > 0 && (
                  <span className="absolute -top-0.5 -right-0.5 min-w-[16px] h-4 px-1 rounded-full bg-secondary text-[9px] font-extrabold flex items-center justify-center tabular animate-pop">
                    {cartCount}
                  </span>
                )}
              </button>
          ...
          <div className="mt-4 flex items-center gap-2">
            <button
              ...
              className="flex-1 h-11 rounded-2xl bg-white text-ink-400 text-[12px] font-medium flex items-center gap-2 px-4 text-right shadow-card"
            >
              <Search size={16} className="text-ink-400" />
              ابحث عن منتج، متجر، أو علامة...
            </button>
            <button
              onClick={() => navigate("search", { q: "", filters: true })}
              className="w-11 h-11 rounded-2xl bg-secondary text-white flex items-center justify-center shadow-accent"
              aria-label="الفلاتر"
            >
      ...
      <div className="flex-1 overflow-y-auto scroll-thin pb-28">
        ...
        <PromoCarousel
          ...
          onAction={(p) => {
            ...
            if (p.action.type === "store")
              return navigate("store", { id: p.action.id });
          ...
          <div className="flex gap-2 overflow-x-auto no-scrollbar -mx-4 px-4 pb-1">
            {CATEGORIES.map((c) => (
              <button
                key={c.id}
                onClick={() => setCat(c.id)}
                className={`h-8 px-3.5 rounded-full text-[12px] font-bold whitespace-nowrap transition ${cat === c.id ? "bg-primary text-white shadow-brand" : "bg-white text-ink-700 border border-ink-200"}`}
              >
                {c.label}
              </button>
            ))}
        ...
        <div className="px-4 pt-5">
          <SectionHeader
            title={`المتاجر المعتمدة في ${CITY}`}
            subtitle="تسوق مباشرة من أشهر متاجر المدينة"
            action="كل المتاجر"
            onAction={() => switchTab("nearbyStores")}
          />
          <div className="grid grid-cols-2 gap-3">
            {STORES.filter((s) => s.verified)
              .slice(0, 2)
              .map((s) => (
                <button
                  key={s.id}
                  onClick={() => navigate("store", { id: s.id })}
                  className="card overflow-hidden text-right active:scale-[0.98] transition"
                >
                  <div className="h-20 bg-ink-100 relative overflow-hidden">
                    {s.cover ? (
                      ...
                      <div className="w-full h-full bg-gradient-to-br from-primary-100 to-secondary-50 flex items-center justify-center">
                        <Store className="text-primary-300" size={30} />
                      </div>
                    )}
                  ...
                  <div className="p-2.5">
                    <p className="text-[12px] font-bold text-ink-900 truncate">
                      {s.name}
                    </p>
        ...
        <div className="px-4 pt-5">
          <SectionHeader
            title="أبرز المنتجات المختارة"
            action="المزيد"
            onAction={() => navigate("search", { q: "" })}
          />
          {featured.length ? (
            <div className="grid grid-cols-2 gap-3">
              {featured.map((p) => (
                <ProductCard
                  key={p.id}
                  product={p}
                  onOpen={(pr) => navigate("product", { id: pr.id })}
                />
              ))}
            </div>
          ) : (
            <div className="card p-6 text-center text-[12px] text-ink-500">
    ...
    <div className="flex-1 flex flex-col bg-ink-50 relative">
      ...
      <div className="bg-white border-b border-ink-100 px-4 py-2.5 flex gap-2 overflow-x-auto no-scrollbar">
        {AREAS.map((a) => (
          <button
            key={a}
            onClick={() => setArea(a)}
            className={`h-8 px-3.5 rounded-full text-[12px] font-bold whitespace-nowrap ${area === a ? "bg-primary text-white" : "bg-ink-100 text-ink-700"}`}
          >
            {a}
          </button>
        ))}
      ...
      {list.length ? (
        <div className="flex-1 overflow-y-auto scroll-thin px-4 py-4 pb-28 space-y-2.5">
          {list.map((s) => (
            <button
              key={s.id}
              onClick={() => navigate("store", { id: s.id })}
              className="w-full card p-3 flex items-center gap-3 text-right active:scale-[0.99] transition"
            >
              <StoreAvatar store={s} size={52} />
              <div className="flex-1 min-w-0">
                <p className="text-[13px] font-bold text-ink-900 truncate flex items-center gap-1">
                  {s.name}{" "}
                  {s.verified && (
                    <ShieldCheck size={13} className="text-primary shrink-0" />
                  )}
                </p>
                <p className="text-[11px] font-medium text-ink-500 flex items-center gap-1">
                  <MapPin size={11} /> {s.area} · <Truck size={11} />{" "}
                  {s.deliveryTime}
                </p>
                <div className="flex items-center gap-2 mt-1">
                  <Chip tone={s.open ? "success" : "danger"}>
                    {s.open ? "مفتوح" : "مغلق"}
                  </Chip>
                  <Rating value={s.rating} />
                </div>
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center px-8 pb-24">
          <div className="w-24 h-24 rounded-3xl bg-ink-100 text-ink-400 flex items-center justify-center mb-5">
            <Store size={40} strokeWidth={1.6} />
          </div>
          <h2 className="text-[20px] font-extrabold">
            لا توجد متاجر في هذا النطاق حالياً
          </h2>
          <p className="text-[12px] text-ink-500 mt-2">
            نعمل على تغطية كافة أحياء {CITY} قريباً. يمكنك تغيير الحي للاطلاع
            على المتاجر المجاورة.
          </p>
          <button
            onClick={() => setArea("الكل")}
...
function StoreProductCard({ product, onOpen }) {
  ...
  const add = (e) => {
    e.stopPropagation();
    if (closed)
      return showToast("المتجر مغلق حالياً ولا يستقبل طلبات جديدة", "danger");
    if (product.stock <= 0)
      return showToast("عذراً، نفدت الكمية من المخزون", "danger");
    if (!requireCustomer(product.shortName)) return; // الزائر: نافذة مطالبة بتسجيل الدخول أو إنشاء حساب عميل
    if (inCart >= product.stock)
      return showToast(`الحد الأقصى المتاح ${product.stock} قطعة`, "danger");
  ...
  return (
    <div
      ...
      className="card p-2.5 cursor-pointer active:scale-[0.98] transition"
    >
      <div className="relative">
        <ProductThumb
          product={product}
      ...
      <h4 className="text-[12px] font-bold text-ink-900 leading-snug line-clamp-2 min-h-[34px] mt-2.5">
        {product.name}
      </h4>
      <div className="mt-2 flex items-center justify-between gap-2">
        ...
        >
          ...
          {inCart > 0 && (
            <span className="absolute -top-1.5 -left-1.5 min-w-[18px] h-[18px] px-1 rounded-full bg-secondary text-[10px] font-extrabold flex items-center justify-center border-2 border-white tabular">
              {inCart}
            </span>
          )}
        </button>
...
export function StoreScreen() {
  ...
  const products = PRODUCTS.filter(
    (p) =>
      !p.deleted &&
      p.storeId === store.id &&
      (store.id !== "st-tech" || state.merchantProducts.includes(p.id)),
  );
    ...
    return (
      <div className="flex-1 flex flex-col bg-ink-50">
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center px-8 pb-6 animate-fade-in">
          <div className="w-24 h-24 rounded-3xl bg-warning-50 text-warning flex items-center justify-center mb-6">
            <PackageX size={42} strokeWidth={1.7} />
          </div>
          <h2 className="text-[22px] font-extrabold text-ink-900">
            لا توجد منتجات متوفرة حالياً
          </h2>
          <p className="text-[13px] font-medium text-ink-500 leading-relaxed mt-3">
            يقوم هذا المتجر بتحديث وتجديد مخزونه التجاري في الوقت الراهن. يرجى
            مراجعة المتاجر الأخرى في نفس الحي.
          </p>
        </div>
        <div className="px-5 pb-4">
          <button
            onClick={() => navigate("nearbyStores", {}, { replace: true })}
      ...
      <div className="flex-1 overflow-y-auto scroll-thin">
        ...
        <div className="relative h-[210px] bg-ink-200 overflow-hidden">
          {store.cover ? (
            ...
            <div className="w-full h-full bg-gradient-to-br from-primary to-primary-400" />
          )}
          ...
          <div className="absolute inset-x-0 top-0">
            ...
            <div className="px-4">
              <button
                onClick={back}
                className="icon-btn bg-white/20 text-white hover:bg-white/30 backdrop-blur"
                aria-label="رجوع"
              >
                <ArrowRight size={18} strokeWidth={2.4} />
              </button>
          ...
          <div className="card p-4 flex items-center gap-3 shadow-elevated">
            <div className="w-[60px] h-[60px] rounded-2xl bg-primary-50 border border-primary-100 flex items-center justify-center shrink-0">
              {store.id === "st-tech" ? (
                ...
                <StoreAvatar store={store} size={52} />
              )}
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5">
                <h1 className="text-[16px] font-extrabold text-ink-900 truncate">
                  {store.name}
                </h1>
                {store.verified && (
              ...
              <p className="text-[12px] font-medium text-ink-500 mt-0.5">
                {store.area}، {store.city}
              </p>
              <div className="flex items-center gap-1.5 mt-2 flex-wrap">
                <Chip tone={store.open ? "success" : "danger"}>
                  ● {store.open ? "معتمد ومفتوح" : "مغلق حالياً"}
                </Chip>
                {store.verified && (
                  <Chip tone="secondary">متجر رسمي موثوق</Chip>
                )}
              </div>
            </div>
          </div>
        ...
        <div className="px-4 pt-3 space-y-3">
          <div className="card p-4">
            <p className="text-[13px] font-extrabold text-ink-900 flex items-center gap-1.5 mb-1.5">
              <Info size={14} className="text-primary" /> عن المتجر
            </p>
            <p className="text-[12px] font-medium text-ink-600 leading-relaxed">
              {store.description}
            </p>
            <div className="grid grid-cols-2 gap-2 mt-3 text-[11px]">
              <div className="rounded-xl bg-ink-50 p-2.5 flex items-center gap-2">
                <div className="w-8 h-8 rounded-lg bg-success-50 text-success flex items-center justify-center shrink-0">
                  <Truck size={15} />
                </div>
                <div className="min-w-0">
                  <p className="text-ink-400 font-medium leading-none">
                    وقت التوصيل المتوقع
                  </p>
                  <p className="font-bold text-ink-900 mt-1 truncate">
                    {store.deliveryTime}
                  </p>
                </div>
              </div>
              ...
              <div className="rounded-xl bg-ink-50 p-2.5 flex items-center gap-2">
                <div className="w-8 h-8 rounded-lg bg-secondary-50 text-secondary flex items-center justify-center shrink-0">
                  <UserRound size={15} />
                </div>
                <div className="min-w-0">
                  <p className="text-ink-400 font-medium leading-none">
              ...
              <a
                ...
                className="rounded-xl bg-ink-50 p-2.5 flex items-center gap-2 active:bg-primary-50 transition"
                aria-label={`اتصال بالمتجر ${store.phone}`}
              >
                ...
                <div className="min-w-0">
                  ...
                  <p className="font-bold text-primary mt-1 tabular" dir="ltr">
                    {store.phone}
                  </p>
                </div>
              </a>
          ...
          <div className="card p-3">
            <div className="flex items-center justify-between mb-2">
              <p className="text-[13px] font-extrabold text-ink-900 flex items-center gap-1.5">
                <MapPin size={14} className="text-secondary" /> موقع المتجر على
                الخريطة
              </p>
        ...
        <div className="px-4 pt-5 pb-28">
          {!store.open && (
            <div
              className="rounded-card bg-danger-50 border border-danger-100 px-3 py-2.5 mb-3 flex items-center gap-2 text-[11px] font-bold text-danger"
              role="status"
            >
              <Clock size={14} className="shrink-0" /> المتجر مغلق حالياً ولا
              يستقبل طلبات جديدة — يمكنك تصفح المنتجات فقط
            </div>
          )}
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-[12px] font-medium text-ink-400 tabular">
          ...
          <p className="text-[11px] font-medium text-ink-400 text-center mt-4">
            الحد الأدنى للطلب من هذا المتجر {fmt(store.minOrder)} ر.ي · التوصيل
            خلال {store.deliveryTime}
          </p>
        </div>
      </div>
...
export function ProductScreen() {
  ...
  const add = () => {
    ...
    if (inCart + qty > product.stock)
      return showToast(`المتاح في المخزون ${product.stock} قطعة فقط`, "danger");
        ...
        <div className="relative h-[300px]" style={{ background: product.bg }}>
          ...
          <div className="absolute inset-x-0 top-0">
            ...
            <div className="px-4 flex items-center justify-between">
              <button
                onClick={back}
                className="icon-btn bg-white/90 shadow-card"
                aria-label="رجوع"
              >
                <ArrowRight size={18} strokeWidth={2.4} />
              </button>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => navigate("cart")}
                  className="relative icon-btn bg-white/90 shadow-card"
                  aria-label="السلة"
                >
                  <ShoppingCart size={17} strokeWidth={2.2} />
                  {cartCount > 0 && (
                    <span className="absolute -top-1 -left-1 min-w-[16px] h-4 px-1 rounded-full bg-secondary text-white text-[9px] font-extrabold flex items-center justify-center tabular">
                      {cartCount}
                    </span>
                  )}
                </button>
        ...
        <div className="px-5 pt-4 pb-32">
          <div className="flex items-center justify-between gap-2">
            <button
              ...
              className="text-[13px] font-bold text-primary flex items-center gap-1 min-w-0"
            >
              <Store size={14} className="shrink-0" />{" "}
              <span className="truncate">{store.name}</span>{" "}
              <ChevronLeft size={14} className="shrink-0" />
            </button>
            <span className="text-[10px] font-bold text-success-700 flex items-center gap-1 whitespace-nowrap">
              <Truck size={12} /> التوصيل {store.deliveryTime}
            </span>
          </div>
          <h1 className="text-[18px] font-extrabold text-ink-900 leading-snug mt-1">
            {product.name}
          </h1>
          <div className="flex items-center gap-2 mt-2 flex-wrap">
            {soldOut ? (
              <Chip tone="danger">نفدت الكمية</Chip>
            ) : (
              <Chip tone="success">متوفر في المخزون ({product.stock})</Chip>
            )}
            {closed && <Chip tone="danger">المتجر مغلق حالياً</Chip>}
            <Chip tone="primary">ضمان التاجر</Chip>
            {product.badge && !soldOut && (
              <Chip tone="secondary">{product.badge}</Chip>
            )}
          </div>
          ...
          <h3 className="text-[15px] font-extrabold text-ink-900 mt-6">
            الوصف والمميزات
          </h3>
          <p className="text-[13px] font-medium text-ink-600 leading-relaxed mt-1.5">
            {product.description}
          </p>
          <h3 className="text-[15px] font-extrabold text-ink-900 mt-6">
            المواصفات الفنية
          ...
          <Stepper
            ...
            maxHint={
              soldOut
                ? false
                : remaining === 0
                  ? `أقصى كمية متاحة — لديك ${inCart} في السلة`
                  : inCart > 0
                    ? `أقصى كمية متاحة ${remaining} (لديك ${inCart} في السلة)`
                    : `أقصى كمية متاحة (${product.stock})`
            }
          />
          <button
            onClick={add}
            className={`flex-1 btn-lg ${soldOut || closed ? "btn-ghost" : "btn-primary"}`}
          >
...
export function SearchScreen() {
  ...
  const activeFilters =
    (cat !== "all" ? 1 : 0) + (maxPrice ? 1 : 0) + (inStockOnly ? 1 : 0);
  const resetFilters = () => {
    setCat("all");
    setMaxPrice(0);
  ...
  const results = useMemo(() => {
    ...
    let list = PRODUCTS.filter((p) => !p.deleted).filter(
      (p) =>
        !t ||
        p.name.includes(t) ||
        p.shortName.includes(t) ||
        CATEGORIES.find((c) => c.id === p.category)?.label.includes(t) ||
        storeById(p.storeId).name.includes(t),
    );
  ...
  const suggested = useMemo(
    () =>
      PRODUCTS.filter((p) => !p.deleted && p.stock > 0)
  ...
  return (
    <div className="flex-1 flex flex-col bg-ink-50 relative">
      ...
      <div className="bg-white border-b border-ink-100 px-4 pb-3">
        <h1 className="text-[18px] font-extrabold text-ink-900 mb-2">
          البحث الذكي
        </h1>
        <div className="flex items-center gap-2">
          <div className="relative flex-1">
            <input
              ...
              className="field pr-11 bg-ink-100"
              autoFocus
            />
            <Search
              size={18}
          ...
          <button
            ...
            className={`relative w-11 h-11 rounded-2xl flex items-center justify-center shrink-0 transition ${showFilters || activeFilters ? "bg-secondary text-white shadow-accent" : "bg-ink-100 text-ink-700"}`}
            aria-label="الفلاتر"
          >
            <SlidersHorizontal size={18} strokeWidth={2.2} />
            {activeFilters > 0 && (
              <span className="absolute -top-1 -left-1 w-4 h-4 rounded-full bg-primary text-white text-[9px] font-extrabold flex items-center justify-center">
                {activeFilters}
              </span>
            )}
          </button>
        ...
        <div className="flex gap-2 mt-3 overflow-x-auto no-scrollbar">
          {[
            ...
            <button
              key={k}
              onClick={() => setSort(k)}
              className={`h-8 px-3.5 rounded-full text-[12px] font-bold whitespace-nowrap ${sort === k ? "bg-primary text-white" : "bg-ink-100 text-ink-700"}`}
            >
              {l}
            </button>
          ))}
              ...
              <div className="flex gap-1.5 flex-wrap">
                {CATEGORIES.map((c) => (
                  <button
                    key={c.id}
                    onClick={() => setCat(c.id)}
                    className={`h-7 px-3 rounded-full text-[11px] font-bold ${cat === c.id ? "bg-secondary text-white" : "bg-white border border-ink-200 text-ink-700"}`}
                  >
                    {c.label}
                  </button>
                ))}
              ...
              <div className="flex gap-1.5 flex-wrap">
                {[
                  ...
                  <button
                    ...
                    className={`h-7 px-3 rounded-full text-[11px] font-bold tabular ${maxPrice === v ? "bg-secondary text-white" : "bg-white border border-ink-200 text-ink-700"}`}
                  >
                    {l}
                  </button>
                ))}
        ...
        {!results ? (
            ...
            <div className="flex flex-wrap gap-2">
              {TRENDING_SEARCHES.map((t) => (
                <button
                  key={t}
                  onClick={() => setQ(t)}
                  className="h-9 px-4 rounded-full bg-white border border-ink-200 text-[12px] font-semibold text-ink-700 hover:border-primary hover:text-primary transition"
                >
                  {t}
                </button>
              ))}
            </div>
            <p className="text-[13px] font-bold text-ink-900 mt-6 mb-2">
              تصفح حسب التصنيف
            </p>
            <div className="grid grid-cols-2 gap-2">
              {CATEGORIES.filter((c) => c.id !== "all").map((c) => (
                <button
                  ...
                  className="card p-3 text-right text-[12px] font-bold text-ink-800 hover:border-primary"
                >
                  {c.label}
                </button>
              ))}
          ...
          <>
            <p className="text-[11px] font-medium text-ink-500 mb-3">
              ...
              {q && (
                <>
                  {" "}
                  لـ "<b className="text-ink-900">{q}</b>"
                </>
              )}
              {activeFilters > 0 && <> · {activeFilters} فلتر مفعّل</>}
            </p>
            <div className="grid grid-cols-2 gap-3">
              {results.map((p) => (
                <ProductCard
          ...
          <div className="flex flex-col items-center text-center pt-6 px-4">
            ...
            <h2 className="text-[20px] font-extrabold text-ink-900">
              لم نعثر على منتجات أو متاجر مطابقة
            </h2>
            <p className="text-[12px] text-ink-500 mt-2 leading-relaxed max-w-[280px]">
              {q ? (
                <>
                  لا توجد نتائج لـ "<b className="text-ink-800">{q}</b>"
                  {activeFilters > 0 && " مع الفلاتر المفعّلة"}.{" "}
                </>
              ) : (
                "لا توجد نتائج مع الفلاتر المفعّلة. "
              )}
              جرّب كلمات أعم، أو تحقق من الإملاء، أو عدّل الفلاتر.
            </p>
            <div className="flex flex-col gap-2 w-full mt-6">
              <button
                ...
                className="w-full btn-primary btn-md"
              >
                <Sparkles size={16} /> العودة إلى المنتجات المقترحة
              </button>
              {activeFilters > 0 && (
                <button
                  onClick={resetFilters}
...
export function FiltersScreen() {
  ...
  const { navigate } = useApp();
  useEffect(() => {
    navigate("search", { q: "", filters: true }, { replace: true });
  }, [navigate]);
  return null;
}
      ...
      <div className="flex-1 overflow-y-auto scroll-thin px-4 py-4 pb-28">
        {list.length ? (
              ...
              <div key={p.id} className="card overflow-hidden">
                ...
                >
                  ...
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      dispatch({ type: "TOGGLE_FAVORITE", productId: p.id });
                      showToast("أُزيل من المفضلة");
                    }}
                    className="absolute top-2 left-2 w-8 h-8 rounded-full bg-white/90 shadow-card flex items-center justify-center text-danger"
                  >
                    <Trash2 size={14} />
                  </button>
                </div>
                <div className="p-2.5">
                  <h4 className="text-[12px] font-bold text-ink-900 line-clamp-2 min-h-[34px]">
                    {p.shortName}
                  </h4>
                  ...
                  <div className="mt-2">
                    <AddToCartButton product={p} full />
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="flex flex-col items-center text-center pt-16 px-6">
            ...
            <p className="text-[12px] text-ink-500 mt-2">
              اضغط على أيقونة القلب في أي منتج لحفظه هنا والعودة إليه لاحقاً.
            </p>
            <button
              onClick={() => switchTab("home")}
              className="btn-primary btn-md mt-6"
