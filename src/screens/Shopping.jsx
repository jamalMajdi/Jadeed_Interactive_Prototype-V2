...
export function Addresses() {
  const { state, dispatch, navigate, canGoBack, switchTab, showToast, back } =
    useApp();
  ...
  const addNew = () => {
    if (form.title.trim().length < 2 || form.details.trim().length < 5)
      return showToast("أكمل اسم العنوان وتفاصيله", "danger");
    const id = `a${Date.now()}`;
    dispatch({
      type: "ADD_ADDRESS",
      address: {
  ...
  return (
    <div className="flex-1 flex flex-col bg-ink-50">
      ...
      <TopBar
        ...
        subtitle={
          canGoBack
            ? `عناوينك المحفوظة في ${CITY}:`
            : "اختر عنوانك أو حدد موقعك الحالي لعرض المتاجر الأقرب إليك"
        }
        code={canGoBack ? "CUS-040" : undefined}
      />
            ...
            {state.addresses.map((a) => {
              ...
              return (
                <button
                  key={a.id}
                  onClick={() => setSel(a.id)}
                  className={`w-full card p-4 flex items-start gap-3 text-right transition ${active ? "border-2 border-primary bg-primary-50/40" : ""}`}
                >
                  <div
                    className={`w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 mt-0.5 ${active ? "bg-primary border-primary text-white" : "border-ink-300"}`}
                  >
                  ...
                  <MapPin
                    size={18}
                    className={active ? "text-primary" : "text-ink-300"}
                  />
                </button>
              );
            })}
        ...
        {adding ? (
          <div className="card p-4 space-y-3 animate-slide-up">
            ...
            <div className="flex gap-2">
              <button onClick={addNew} className="flex-1 btn-primary btn-md">
                حفظ العنوان
              </button>
              <button
          ...
          <button
            ...
            className="w-full h-12 rounded-card border-2 border-dashed border-primary-300 text-primary text-[13px] font-bold flex items-center justify-center gap-2 hover:bg-primary-50 transition"
          >
      ...
      <div className="px-5 pb-4 pt-2 bg-ink-50">
        <button onClick={confirm} className="w-full btn-primary btn-lg">
        ...
        <div className="px-4 pb-4 relative">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                {unread > 0 && (
                  <span className="absolute -top-0.5 -right-0.5 w-4 h-4 rounded-full bg-secondary text-[9px] font-extrabold flex items-center justify-center">
                    {unread}
                  </span>
                )}
              </button>
          ...
          <div className="mt-4 flex items-center gap-2">
            ...
            <button
              ...
              className="w-11 h-11 rounded-2xl bg-secondary text-white flex items-center justify-center shadow-secondary"
              aria-label="الفلاتر"
            >
      ...
      <div className="flex-1 overflow-y-auto scroll-thin pb-28">
        ...
        <div className="px-4 pt-4">
          <div className="relative overflow-hidden rounded-modal bg-gradient-to-l from-secondary to-secondary-400 text-white p-4 shadow-secondary">
            ...
            <p className="text-[12px] font-medium text-white/90">
              على الأجهزة الإلكترونية والساعات
            </p>
            <button
              onClick={() => setCat("electronics")}
              className="mt-3 h-8 px-4 rounded-full bg-white text-secondary text-[12px] font-bold"
            >
              تسوق الآن
            </button>
          </div>
          ...
          <div className="flex gap-2 overflow-x-auto no-scrollbar -mx-4 px-4 pb-1">
            {CATEGORIES.map((c) => (
              <button
                key={c.id}
                onClick={() => setCat(c.id)}
                className={`h-8 px-3.5 rounded-full text-[12px] font-bold whitespace-nowrap transition ${cat === c.id ? "bg-primary text-white shadow-primary" : "bg-white text-ink-700 border border-ink-200"}`}
              >
                {c.label}
              </button>
            ))}
        ...
        <div className="px-4 pt-5">
          ...
          <button
            ...
            className="mt-3 w-full h-10 rounded-xl border border-primary/20 bg-primary-50 text-primary text-[12px] font-extrabold flex items-center justify-center gap-2 active:scale-[0.98] transition"
            aria-label="عرض المتاجر القريبة منك"
          >
...
// ─────────────────────────────────────────────────────────────
//  تفاصيل المتجر + قائمة منتجاته (CUS-020 للحالة الفارغة)
// ─────────────────────────────────────────────────────────────
export function StoreScreen() {
  const { current, navigate, back, state } = useApp();
  ...
  return (
    <div className="flex-1 flex flex-col bg-ink-50">
      <div className="relative h-44 bg-ink-200 overflow-hidden">
        {store.cover ? (
          <img
            src={store.cover}
            alt=""
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-ink-900/50 to-transparent" />
        <div className="absolute inset-x-0 top-0">
          <StatusBar light />
          <div className="px-4">
            <button
              onClick={back}
              className="icon-btn bg-white/20 text-white hover:bg-white/30"
              aria-label="رجوع"
            >
              <ArrowRight size={18} strokeWidth={2.4} />
            </button>
        ...
        <div className="card p-4 flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-1.5 mt-2 flex-wrap">
              ...
              <Chip tone="ink">
                <Clock size={10} /> {store.prepTime}
              </Chip>
            </div>
          </div>
        </div>
        <p className="text-[11px] font-medium text-ink-500 leading-relaxed mt-3 px-1">
          {store.description}
        </p>
      </div>
      <div className="px-4 pt-4 flex-1 overflow-y-auto scroll-thin pb-6">
        <SectionHeader
          title="قائمة منتجات المتجر"
          subtitle={`${products.length} منتج · الحد الأدنى للطلب ${fmt(store.minOrder)} ر.ي`}
        />
        {products.length ? (
          <div className="grid grid-cols-2 gap-3">
            {products.map((p) => (
          ...
          <div className="flex flex-col items-center text-center pt-10 px-6">
            <div className="w-24 h-24 rounded-3xl bg-warning-50 text-warning flex items-center justify-center mb-5">
...
export function ProductScreen() {
  const { current, navigate, back, dispatch, showToast, cartCount, state } =
    useApp();
    ...
    <div className="flex-1 flex flex-col bg-white">
      <div className="flex-1 overflow-y-auto scroll-thin">
        ...
        <div className="px-5 pt-4 pb-32">
          <button
            ...
            className="text-[13px] font-bold text-primary flex items-center gap-1"
          >
          ...
          <div className="card p-4 mt-4 flex items-center justify-between bg-ink-50">
            ...
            {discount > 0 && (
              <Chip tone="solidSecondary" className="!h-7 !text-[11px]">
                وفر {discount}%
              </Chip>
            )}
          </div>
      ...
      <div className="absolute inset-x-0 bottom-0 bg-white/95 backdrop-blur border-t border-ink-100 px-5 pt-3 pb-2">
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={add}
            className={`flex-1 btn-lg ${soldOut ? "btn-ghost" : "btn-primary"}`}
          >
      ...
      <div className="flex-1 overflow-y-auto scroll-thin px-4 py-4 pb-28">
        {!results ? (
          ...
          <div className="flex flex-col items-center text-center pt-16 px-6">
            ...
            <p className="text-[12px] text-ink-500 mt-2">
              تأكد من كتابة الكلمات بشكل صحيح، أو جرب البحث بكلمات عامة أخرى.
            </p>
            <button
              onClick={() => setQ("")}
  ...
  return (
    <div className="flex-1 flex flex-col bg-ink-50">
      ...
      <div className="flex-1 px-5 py-4">
        <p className="text-[13px] font-bold text-ink-900 mb-3">
          خيارات الترتيب المعتمدة:
        </p>
        <div className="space-y-2.5">
          {[
            [
              ...
              MapPin,
              "الأقرب مسافة أولاً",
              `حسب موقعك الحالي في ${CITY}`,
            ],
            [
              "cheap",
              Filter,
              "الأرخص سعراً أولاً",
              "من السعر الأقل إلى الأعلى",
            ],
            ["rated", Sparkles, "الأعلى تقييماً", "حسب تقييمات العملاء"],
          ].map(([k, Icon, t, d]) => {
            const active = sel === k;
            return (
              <button
                key={k}
                onClick={() => setSel(k)}
                className={`w-full card p-4 flex items-center gap-3 text-right ${active ? "border-2 border-primary bg-primary-50/40" : ""}`}
              >
                <Icon
                  size={20}
                ...
                <div className="flex-1">
                  <div
                    className={`text-[14px] font-bold ${active ? "text-primary" : "text-ink-900"}`}
                  >
                    {t}
                  </div>
                ...
                <div
                  className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${active ? "bg-primary border-primary text-white" : "border-ink-300"}`}
                >
                  {active && <Check size={13} strokeWidth={3.5} />}
                </div>
              </button>
