...
import { MapPin, Navigation, Check, Plus, ChevronLeft, Search, SlidersHorizontal, Bell, ShoppingBag, Sparkles, ArrowRight, Store, X, Heart, Trash2, ShieldCheck, Clock, LocateFixed, MapPinOff, ShoppingCart, Filter } from 'lucide-react'
    ...
    <div className="flex-1 flex flex-col bg-ink-50">
      <div className="relative h-44 bg-ink-200 overflow-hidden">
        ...
        <div className="absolute inset-x-0 top-0">
          <StatusBar light />
          <div className="px-4">
            <button onClick={back} className="icon-btn bg-white/20 text-white hover:bg-white/30" aria-label="رجوع"><ArrowRight size={18} strokeWidth={2.4} /></button>
          </div>
        </div>
      </div>
      <div className="px-4 -mt-10 relative">
        <div className="card p-4 flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] font-medium text-ink-500">{store.area}، {store.city} · <Rating value={store.rating} count={store.reviews} /></p>
            <div className="flex items-center gap-1.5 mt-2 flex-wrap">
              ...
              <Chip tone="ink"><Clock size={10} /> {store.prepTime}</Chip>
            </div>
          </div>
        </div>
        <p className="text-[11px] font-medium text-ink-500 leading-relaxed mt-3 px-1">{store.description}</p>
      </div>
      <div className="px-4 pt-4 flex-1 overflow-y-auto scroll-thin pb-6">
        <SectionHeader title="قائمة منتجات المتجر" subtitle={`${products.length} منتج · الحد الأدنى للطلب ${fmt(store.minOrder)} ر.ي`} />
        {products.length ? (
          <div className="grid grid-cols-2 gap-3">
            {products.map((p) => (
              <ProductCard key={p.id} product={p} onOpen={(pr) => navigate('product', { id: pr.id })} />
          ...
          <div className="flex flex-col items-center text-center pt-10 px-6">
            <div className="w-24 h-24 rounded-3xl bg-warning-50 text-warning flex items-center justify-center mb-5"><ShoppingBag size={40} strokeWidth={1.6} /></div>
            <h2 className="text-[20px] font-extrabold">لا توجد منتجات متوفرة حالياً</h2>
            <p className="text-[12px] text-ink-500 mt-2">يقوم هذا المتجر بتحديث وتجديد مخزونه التجاري في الوقت الراهن. يرجى مراجعة المتاجر الأخرى في نفس الحي.</p>
            <button onClick={() => navigate('nearbyStores', {}, { replace: true })} className="btn-primary btn-md mt-6">تصفح متاجر أخرى</button>
          </div>
        )}
      </div>
      <HomeIndicator />
